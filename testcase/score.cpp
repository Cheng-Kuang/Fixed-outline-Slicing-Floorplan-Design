...
vector<int> simulatedAnneal(const Data& in, vector<int> expr, const vector<int>& rot, int W, int H, vector<pair<int, int>> pos, uint64_t seed) {
    ...
    // 580 seconds
    const auto timeLimit = chrono::seconds(60);
...
//==== Main ====  
int main() {
    const vector<string> files = { "public1.txt", "public2.txt", "public3.txt" };
    const double baselines[3] = { 248807.0, 495065.0, 696662.0 };
    ...
    for (uint64_t seed = 0; seed < 1000; ++seed) {
        ...
        for (int ci = 0; ci < 3; ++ci) {
            Data in;
            if (!readInput(files[ci], in)) continue;

            // compute outline
            long long A = in.totalBlockArea;
            double deadRatio = 0.1;
            int W = (int)ceil(sqrt((double)A * (1 + deadRatio)));
            int H = W;

            // initial LB2 placement → pos, rot and get initial Polish expr
            vector<pair<int,int>> pos;
            vector<int> rot;
            normalizeTall(in, rot);
            vector<int> initExpr = initialLB2(in, W, H, pos, rot);

            // simulated annealing with this seed
            vector<int> bestExpr = simulatedAnneal(in, initExpr, rot, W, H, pos, seed);

            // evaluate bestExpr
            vector<pair<int,int>> bestPos;
            bool ok = evalExpr(bestExpr, in, W, H, bestPos);
            if (!ok) {
                // fallback to initial placement
                bestPos = pos;
            }

            double wl = computeHPWL(in, bestPos);
            totalScore += (baselines[ci] - wl) / baselines[ci];
        }
		cout<<seed << " " << totalScore << "\n";
        scoreOut << seed << " " << totalScore << "\n";
    }
